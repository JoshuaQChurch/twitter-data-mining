...
def parser(args):
    ...
    # Check if the user supplied the 'mine' flag
    elif any(flag in args for flag in flags["mine"]):
        query = {} 
        hashtag_file = None 
        hashtag_list = None
        ...
        if len(file_indices) > 1 or len(list_indices) > 1:
            print("ERROR: Cannot provide more than (1) hashtag arg")
            sys.exit(-1)
    ...
    # Check if the user supplied the 'remine' flag
    elif any(flag in args for flag in flags["remine"]):
        query = {} 
        hashtag_file = None 
        hashtag_list = None
        ...
        # Cannot mix the mine and extract args
        if any(flag in args for flag in flags["extract"]): 
            print("ERROR: Cannot use the remine and extract commands concurrently.")
            sys.exit(-1)

        elif any(flag in args for flag in flags["mine"]): 
            print("ERROR: Cannot use the remine and mine commands concurrently.")
            sys.exit(-1)

        # Cannot mix the mine and analyze args
        elif any(flag in args for flag in flags["analyze"]):
            print("ERROR: Cannot use the remine and analyze commands concurrently.")
            sys.exit(-1)
        ...
        # If the user supplied a start date
        if since_indices:
            index = since_indices[0]
            if index > 0:
                since = args[index - 1].split('-')
        ...
        # If the user supplied a end date 
        if until_indices:
            index = until_indices[0]
            if index > 0:
                until = args[index - 1].split('-')
        ...
        # If the user supplied a tweet limit
        if limit_indices:
            index = limit_indices[0]
            if index > 0:
                tweet_limit = args[index - 1]
        ...
        # update the hashtags variable
            ...
            print("ERROR: Must provide either a hashtag file or list to remine.")
            sys.exit(-1)
